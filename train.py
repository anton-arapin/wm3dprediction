...
from models import CDiT_models, ActionEmbedder
...
def main(args):
    ...
    ##############################################################
    # we need to change this network to take in 4 channels instead of 3
    #tokenizer.encoder.conv_in
    with torch.no_grad():
        new_conv1 = nn.Conv2d(
            in_channels=4, # Change input channels to 4
            out_channels=tokenizer.encoder.conv_in.out_channels,
            kernel_size=tokenizer.encoder.conv_in.kernel_size,
            stride=tokenizer.encoder.conv_in.stride,
            padding=tokenizer.encoder.conv_in.padding
        )
        new_conv1 = new_conv1.to(tokenizer.device)
        new_conv1.bias.data.copy_(tokenizer.encoder.conv_in.bias.data)

        # 2. Copy the weights from the first 3 channels
        new_conv1.weight[:, :3, :, :] = tokenizer.encoder.conv_in.weight.clone()

        # 3. Initialize the weights for the 4th channel (e.g., by copying the red channel)
        new_conv1.weight[:, 3:, :, :] = tokenizer.encoder.conv_in.weight[:, 0:1, :, :].clone()

        # Set the new layer in the model
        tokenizer.encoder.conv_in = new_conv1
    ##############################################################
    latent_size = config['image_size'] // 8

    assert config['image_size'] % 8 == 0, "Image size must be divisible by 8 (for the VAE encoder)."
    num_cond = config['context_size']
    # Create CDiT model, Autoregression model and action embedding model here
    #############################################################
    action_encoder_model, model, cvit_model = CDiT_models[config['model']](context_size=num_cond, input_size=latent_size, in_channels=4)
    model = model.to(device)
    action_encoder_model = action_encoder_model.to(device)
    ...
    # chain all models to be optimized
    all_params = chain(model.parameters(), action_encoder_model.parameters(), cvit_model.parameters())
    opt = torch.optim.AdamW(all_params, lr=lr, weight_decay=0)
    ...
    if args.torch_compile:
        ...
        action_encoder_model = torch.compile(action_encoder_model)
        cvit_model = torch.compile(cvit_model)
    model = DDP(model, device_ids=[device])
    action_encoder_model = DDP(action_encoder_model, device_ids=[device])
    cvit_model = DDP(cvit_model, device_ids=[device])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t in loader:
            ...
            ###############################
            # add mock 0s for the one depth channel for testing purposes
            padding = torch.zeros(x.shape[0], x.shape[1], 1, x.shape[3], x.shape[4]).to(x.device)
            ...
            with torch.amp.autocast('cuda', enabled=bfloat_enable, dtype=torch.bfloat16):
                ...
                y = y.flatten(0, 1)
                y = action_encoder_model(y)
                rel_t = rel_t.flatten(0, 1)
                
                t = torch.randint(0, diffusion.num_timesteps, (x_start.shape[0],), device=device)
                model_kwargs = dict(y=y, x_cond=x_cond, rel_t=rel_t)
                print("before calling diffusion function")
                loss_dict = diffusion.training_losses(model, cvit_model, x_start, t, model_kwargs)
