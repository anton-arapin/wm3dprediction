...
class ActionEmbedder(nn.Module):
    ...
    def initialize_weights(self):
        nn.init.normal_(self.x_emb.mlp[0].weight, std=0.02)
        nn.init.normal_(self.x_emb.mlp[2].weight, std=0.02)

        nn.init.normal_(self.y_emb.mlp[0].weight, std=0.02)
        nn.init.normal_(self.y_emb.mlp[2].weight, std=0.02)

        nn.init.normal_(self.angle_emb.mlp[0].weight, std=0.02)
        nn.init.normal_(self.angle_emb.mlp[2].weight, std=0.02)
...
class FinalAutoregressiveLayer(nn.Module):
    """
    The final layer of CViT.
    """
    def __init__(self, hidden_size, patch_size, out_channels):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
...
class CDiT(nn.Module):
    ...
    def forward(self, x, t, y, x_cond, rel_t, x_autoregressed):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps -- this is diffusion timestep
        y: (N,) tensor of class labels -- this is actions
        rel_t: (N,) -- this is timeshift i.e. how far into the future (or past, but why?) to predict
        """
        breakpoint()
        ...
        #y = self.y_embedder(y) 
        time_emb = self.time_embedder(rel_t[..., None])  # this is the "timeshift" and it can be negative. I don't understand the utility of this being negative
        c = t + time_emb + y # if training on unlabeled data, dont add y.

        for block in self.blocks:
            #x = block(x, c, x_cond)
            x = block(x, c, x_autoregressed)
...
class CViTBlock(nn.Module):
    """
    A CViT block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
    ...
    def forward(self, x, c, x_cond):
        shift_ca_x, scale_ca_x, gate_ca_x, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        x = x + gate_ca_x.unsqueeze(1) * self.cttn(query=modulate(self.norm2(x), shift_ca_x, scale_ca_x), key=x_cond, value=x_cond, need_weights=False)[0]
        x = x + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm3(x), shift_mlp, scale_mlp))
        return x
...
class CViT(nn.Module):
    ...
    def __init__(
        ...
        num_patches = self.x_embedder.num_patches
        self.pos_embed = nn.Parameter(torch.zeros(self.context_size + 1, num_patches, hidden_size), requires_grad=True) # for context and for predicted frame
        self.blocks = nn.ModuleList([CViTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)])
        #self.final_layer = FinalLayer(hidden_size, patch_size, self.out_channels)
        self.final_autoregressive_layer = FinalAutoregressiveLayer(hidden_size, patch_size, self.out_channels)
        self.initialize_weights()
    ...
    def initialize_weights(self):
        ...
        # Zero-out output layers:
        nn.init.constant_(self.final_autoregressive_layer.adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_autoregressive_layer.adaLN_modulation[-1].bias, 0)
    ...
    def forward(self, x, y, condition):
        """
        Forward pass of DiT.
        x: (N, K, C, H, W) tensor of spatial inputs (images or latent representations of images) where K is the number of context observations
        y: (N,) tensor of class labels -- this is actions/poses corresponding to the observations in x
        condition: (N,) the condition for CViT block -- this is the next action in our case
        """
        breakpoint()
        print("inside autoregression cvit model forward")
        x = self.x_embedder(x.flatten(0, 1)).unflatten(0, (x.shape[0], x.shape[1])) + self.pos_embed[:self.context_size]  # (N, T, D), where T = H * W / patch_size ** 2.flatten(1, 2)
        x = x.flatten(1, 2)

        for block in self.blocks:
            x = block(x, y, condition)
        #x1 = self.final_layer(x, y)
        x = self.final_autoregressive_layer(x, y)
        breakpoint()
        #x = self.unpatchify(x)
        return x
...
def CDiT_S_2(**kwargs):
    hidden_size = 384
    action_embedder_model = ActionEmbedder(hidden_size)
    cdit_model = CDiT(depth=12, hidden_size=hidden_size, patch_size=2, num_heads=6, **kwargs)
    cvit_model = CViT(depth=8, hidden_size=hidden_size, patch_size=2, num_heads=6, **kwargs)
    return action_embedder_model, cdit_model, cvit_model
